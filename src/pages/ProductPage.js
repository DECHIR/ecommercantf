        ...
        <Container className="pt-4" style={{ position: "relative" }}>
            <Row>
                ...
                <Col lg={6} className="pt-4">
                    ...
                    <p>
                        <Badge bg="primary"> Quantite : {product.quantite}</Badge>
                    </p>
                    <p className="product__price">{product.price}DZD</p>
                    <p style={{ textAlign: "justify" }} className="py-3">
                        <strong>Description:</strong> {product.description}
                    </p>
                    {user &&  (
                        <ButtonGroup style={{ width: "90%" }}>
                            <Button size="lg" onClick={() => addToCart({ userId: user._id, productId: id, price: product.price, image: product.pictures[0].url, quantite: nombre })}>
                            Ajouter au Panier
                            </Button> 
                        </ButtonGroup>
                    )}
